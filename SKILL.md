...
- 对 media 文件会先尝试直接转写，失败时再回退到 `ffmpeg` 抽音轨后转写
...
- 提供 `npm run setup-asr`，辅助安装 `ffmpeg` / `whisper-cli` 并生成本地转写命令
...
- 音频与 media 当前为“下载 + 可选本地转写”模式；media 会在直接转写失败时回退到 `ffmpeg` 抽音轨，但更复杂容器格式仍可能需要继续完善
